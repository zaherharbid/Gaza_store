<?php
return [
    'hero' => 'التسوق الفريد والموثوق.',
    'hero_text' => 'نحن نضمن لكم الجودة والخدمة الممتازة.',
    'hero_btn' => 'تسوق الان',
    'hero_discription' => 'واكتشف أسلوبك الخاص!',
    'nav' => [
        'login' => 'تسجيل دخول',
        'sign up' => 'اشتراك جديد',
        'logout' => 'تسجيل خروج',
    ],
    'footer' => [
        'copyright' => 'جميع الحقوق محفوظة © 2025.',
        'followUs' => 'تابعنا',
        'facebook' => 'فيسبوك',
        'twitter' => 'تويتر',
        'instagram' => 'انستغرام',
        'linkedin' => 'لينكدان',
        'contactUs' => 'تواصل معنا',
        'email' => 'emailaddress@mail.com',
        'customerEmail' => ' بريدك الالكتروني',
        'subscribe' => 'اشترك في متجرنا',
        'terms&conditions' => 'الشروط والأحكام',
        'privacyPolicy' => 'سياسة الخصوصية',
    ],
    'types_subtitle' => 'كل الأنواع',
    'types_title' => 'نمتلك كل الأنواع من المنتجات',
    'types_desc' => 'كل الانواع التي تلزمك لكي تبدو بكامل اناقتك موجودة هنا ',
    'products_subtitle' => 'ماذا نقدم',
    'products_title' => 'المنتجات الخاصة بنا دائما مميزة',
    'products_desc' => 'المنتجات الخاصة بهذا النوع ',
    'buy_now' => 'اشتري الأن ',
    'view_details' => 'عرض التفاصيل',
    'product_name' => 'اسم المنتج : ',
    'product_type' => 'نوع المنتج : ',
    'product_price' => 'سعر المنتج :  ',
    'product_description' => 'وصف المنتج : ',
    'product_share' => 'مشاركة عبر وسائل التواصل الاجتماعي : ',
    'ratings&views' => 'التقييمات والآراء',
    'all_product' => 'جميع منتجاتنا',
    'view_all' => 'عرض الكل',
    'recent_products' => 'أحدث المنتجات',
    'related' => 'منتجات ذات صلة',
    'why_are' => ' لماذا نحن خيارك الأفضل !؟',
    'diverse_products' => 'منتجات متنوعة',
    'diverse_products_desc' => 'ملابس رسمية وغير رسمية لتلبية كافة احتياجاتك.',
    'high_quality' => 'جودة عالية',
    'high_quality_desc' => 'نحن نختار مواد عالية الجودة لكل قطعة ملابس.',
    'honesty' => 'الصدق والشفافية',
    'honesty_desc' => 'المنتجات كما هو موضح، ولا مفاجآت.',
    'competitive' => 'أسعار تنافسية',
    'competitive_desc' => 'أسعار معقولة لجميع الميزانيات.',
    'customer' => 'خدمة العملاء',
    'customer_desc' => 'فريقنا جاهز لمساعدتك في الاختيار بحكمة.',
    'blogs' => 'الأخبار و المقالات',
    'blogs_title' => 'المقالات الخاصة بنا ',
    'blogs_desc' => 'استعد لموسم الصيف مع أحدث صيحات الموضة لعام 2025.',
    'read_more' => 'قراءة المزيد ..',
    'search_result' => ' نتائج البحث عن',










];
